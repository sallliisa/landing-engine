  ...
  <div class="flex flex-col gap-lg justify-between px-6 lg:px-8 py-3 lg:py-9 w-full h-full">
    ...
    {#if section.data.content.title || section.data.content.subtitle || section.data.content.description}
      <div class="flex flex-col gap-sm">
        ...
        <p class="text-sm translate-y-[28px] opacity-0 group-hover/carouselItem:translate-y-0 group-hover/carouselItem:opacity-100 transition-all">{section.data.content.url_text || m.learn_more()} <i class="ml-1 ri-arrow-right-up-line"></i></p>
      </div>
    {/if}
  </div>
{/snippet}