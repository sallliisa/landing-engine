<script lang="ts">
  ...
  const contentAlignClassMap = {
    ...
    horizontal: {
      container: 'flex-col md:flex-row items-center md:items-center justify-between',
      content: {
    ...
    <div class="flex flex-col gap-lg w-full {section.meta.content_align === 'horizontal' && section.data.gallery.length > 0 ? 'md:w-1/2' : 'w-full'} {section.data.gallery.length > 0 ? '' : 'pb-6 md:pb-0'} {(contentAlignClassMap as any)[section.meta.content_align].content.container}">
      <div class="flex flex-col gap-base">
        <div class="flex flex-col gap-xs">
          <p class="text-sm {(contentAlignClassMap as any)[section.meta.content_align].content.content}">{section.data.content.subtitle}</p>
          <p class="text-2xl md:text-3xl font-bold {(contentAlignClassMap as any)[section.meta.content_align].content.content}">{section.data.content.title}</p>
        </div>
        {#if section.data.content.description}
          <div class="w-full">
            <p class="rtf-content m-base max-w-[80ch] {(contentAlignClassMap as any)[section.meta.content_align].content.content}">{@html section.data.content.description}</p>
          </div>
        {/if}
      </div>
    ...
    {#if section.data.gallery.length > 0}
      <div class="flex h-full items-center {section.meta.content_align === 'horizontal' ? 'md:w-1/2' : 'w-full'} {(contentAlignClassMap as any)[section.meta.content_align].gallery.container}">
        {#each section.data.gallery as item}
          <div class="flex flex-col gap-sm px-6 py-4 md:px-8 w-full md:w-auto lg:px-12 {(contentAlignClassMap as any)[section.meta.content_align].gallery.content.container}">
